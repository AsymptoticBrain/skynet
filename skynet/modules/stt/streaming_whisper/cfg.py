...
if whisper_backend == 'local':
    ...
    log.info(f'Backend: local (faster-whisper)')
    log.info(f'Model: {path_or_model_name}')
    log.info(f'Multilingual: {model.model.is_multilingual}')
else:
    log.info('====== WHISPER MODEL INFO ======')
    log.info(f'Backend: remote (OpenAI-compatible)')
    log.info(f'Endpoint: {whisper_remote_url}')
    log.info(f'Model: {whisper_remote_model}')
