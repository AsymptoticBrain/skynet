...
enabled_modules = set(
    os.environ.get(
        'ENABLED_MODULES', 'summaries:dispatcher,summaries:executor,customer_configs,streaming_whisper'
    ).split(',')
)
...
# Max accepted upload size for the batch (POST /v1/audio/transcriptions) endpoint.
# Default 500 MB — comfortably covers the 100-min / 1 GB raw recording cap once
# compressed to opus/m4a, which is what Jibri (and most browser recorders) emit.
whisper_batch_max_upload_bytes = int(os.environ.get('WHISPER_BATCH_MAX_UPLOAD_BYTES', 500 * 1024 * 1024))
